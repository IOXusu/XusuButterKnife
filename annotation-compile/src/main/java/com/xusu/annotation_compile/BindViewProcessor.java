package com.xusu.annotation_compile;
...
import com.xusu.annotation_lib.BindView;
...
public class BindViewProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        bindViewMap.forEach(new BiConsumer<TypeElement, List<BindViewInfo>>() {
            @Override
            public void accept(TypeElement typeElement, List<BindViewInfo> bindViewInfos) {
...
//        generateCodeByStringBuffer(bindViewMap);
    ...
    private void generateCodeByStringBuffer(Map<TypeElement, List<BindViewInfo>> bindViewMap) {
        bindViewMap.forEach(new BiConsumer<TypeElement, List<BindViewInfo>>() {
            @Override
            public void accept(TypeElement typeElement, List<BindViewInfo> bindViewInfos) {
                BindViewProcessor.this.generateJavaClassBySb(typeElement, bindViewInfos);
            }
        });
    }
    ...
    private void generateJavaClassBySb(TypeElement typeElement, List<BindViewInfo> bindViewInfos) {
        try {
            ...
            sb.append("import com.xusu.butterknifelib.ViewBinder;\n");
    ...
    private void generateCodeByJavaPoet(Map<TypeElement, List<BindViewInfo>> bindViewMap) {
        bindViewMap.forEach(new BiConsumer<TypeElement, List<BindViewInfo>>() {
            @Override
            public void accept(TypeElement typeElement, List<BindViewInfo> bindViewInfos) {
                BindViewProcessor.this.generateJavaClassByJavaPoet(typeElement, bindViewInfos);
            }
        });
    }
...
//    }
    ...
    private void generateJavaClassByJavaPoet(TypeElement typeElement, List<BindViewInfo> bindViewInfos) {
        ...
        ClassName viewBinder = ClassName.bestGuess("com.xusu.butterknifelib.ViewBinder");
